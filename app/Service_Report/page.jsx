...
const Service_Report = () => {
  ...
  const getreport = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/service/${Range}`);
      if (response.ok) {
  ...
  const getreportforcount = async () => {
    ...
    try {
      const response = await fetch(`/api/service_count/${count}`);
      if (response.ok) {
        const tractors = await response.json()
        console.log("Success")
        setRows(tractors); // Assuming response is like { success: true, data: [...] }
  ...
  return (
    <div className='flex flex-col h-dvh'>
        <div className=' py-2 bg-white content-center h-12 flex w-full gap-3 px-3 '>
          ...
          <Link className=' text-white bg-blue-600 hover:bg-blue-900 py-1 px-6 rounded-md' href={"/Service_Invoice"}>New Service</Link>
        </div>
              ...
              {rows.map((row) => (
                <tr key={row._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    ...
                    <Link className="text-blue-600 hover:text-blue-900" href={`/Service_Invoice/Invoice?id=${row._id}`}>
                      Invoice
                    </Link>
                  </td>
                </tr>
